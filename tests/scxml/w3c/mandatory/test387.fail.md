...
statemachine.exceptions.InvalidDefinition: Failed to create state machine class: 's0HistDeep' from definition: {'states': {'s0': {'states': [State('S01', id='s01', value='s01', initial=False, final=False), State('S02', id='s02', value='s02', initial=False, final=False)]}, 's1': {'initial': True}, 'pass': {'final': True, 'enter': [ExecuteBlock(ExecutableContent(actions=[LogAction(label='Outcome', expr="'pass'")]))]}, 'fail': {'final': True, 'enter': [ExecuteBlock(ExecutableContent(actions=[LogAction(label='Outcome', expr="'fail'")]))]}}}
