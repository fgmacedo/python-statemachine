...
{ref}`queued mode (RTC)` to be compliant with the specs, where the {ref}`event` is put on a queue before processing.
You can also configure your state machine to run back in {ref}`synchronous mode`, where the {ref}`event` will be run immediately and nested events will be chained.
...
{ref}`synchronous mode`. Be aware that we may remove the {ref}`synchronous mode` in the future.
