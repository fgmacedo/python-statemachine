...
class AsyncEngine:
    ...
    async def _processing_loop(self):
        ...
        try:
            ...
            while self.sm._external_queue:
                ...
                try:
                    ...
                    if first_result is self.__sentinel:
                        first_result = result
                except Exception:
        ...
        return first_result if first_result is not self.__sentinel else None
    ...
    async def _trigger(self, trigger_data: TriggerData):
        ...
        for transition in state.transitions:
            ...
            await self.sm._get_callbacks(transition.validators.key).acall(*args, **kwargs)
            if not await self.sm._get_callbacks(transition.cond.key).aall(*args, **kwargs):
                continue
    ...
    async def _activate(self, event_data: EventData):
        ...
        result = await self.sm._get_callbacks(transition.before.key).acall(*args, **kwargs)
        if source is not None and not transition.internal:
            await self.sm._get_callbacks(source.exit.key).acall(*args, **kwargs)

        result += await self.sm._get_callbacks(transition.on.key).acall(*args, **kwargs)
        ...
        if not transition.internal:
            await self.sm._get_callbacks(target.enter.key).acall(*args, **kwargs)
        await self.sm._get_callbacks(transition.after.key).acall(*args, **kwargs)
